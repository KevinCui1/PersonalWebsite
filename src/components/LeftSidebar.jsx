...
export default function LeftSidebar({ activeSection }) {
  ...
  return (
    <aside
      className="hidden lg:flex fixed left-0 top-0 h-screen flex-col items-center z-40 px-8 py-8"
      style={{ width: '200px' }}
    >
      {/* Nav Links */}
      <nav className="flex flex-col gap-6 flex-1 justify-center">
        {NAV_ITEMS.map(({ num, label, id }) => (
          ...
          >
            <span style={{ color: 'var(--accent)', fontFamily: 'monospace', fontSize: '0.7rem' }}>
              {num}.
            </span>
            <span>{label}</span>
          </a>
      ...
      {/* Social Icons + Line */}
      <div className="flex flex-col items-center gap-5" style={{ paddingBottom: 0 }}>
        <a
